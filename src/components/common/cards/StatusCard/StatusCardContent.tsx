...
export interface StatusCardContentProps {
  index?: number;
  title?: any;
  cover?: {
    card: {
        ...
        <div className={`flex flex-col items-end`}>
          ...
          <>
            <div className="flex infotext flex justify-end small dark gap-[5px]">
              ...
              <img
               src={`${currentIcon === `${index}` ? "/images/info-green-icon.svg" : "/images/Maskd (2).svg"}`}
               alt=""
               width={10}
               height={10}
               onMouseEnter={()=>{
      ...
      <div className="flex justify-between cover-content-x">
        <div>
          {table?.rows.map(({ ...rest }, index) => (
            ...
            >
              <InfoText key={index} {...rest} variant="small" index={`${index}-`+`${CardNo}1`}/>
            </div>
          ))}
        </div>
        <div>
              ...
              <>
                <div className="hidden sm:flex flex-col justify-center items-center">
                  <div className="mb-[20px] flex gap-[5px]">
                    <span className="text-[#606166] fw-500 fs-12 lh-14">{table?.rows[index].text}</span>
                    {
                  ...
                  <div><span className="fw-500 fs-12 lh-14">{table?.columns[index]}</span></div>
                </div>
              </>
            ))
          }
