...
function Input(props: InputProps) {
  ...
  return (
    <div className="border border-dark-75 p-[20px] md:p-[24px] dark:bg-white dark:box-border gap-1 md:w-[360px]">
      <div className={`dark:bg-light-200 flex bg-[#2A2B31] justify-between rounded py-2.5 px-5 items-center gap-4 flex-wrap ${className}`}>
        <div className="flex items-center gap-4 flex-wrap">
          <span className={` prp-number-fix w-[60px] 
          ${defaultValue === '00.00' ? 
              "text-[#42434B]"
            : `${theme === 'dark' ? `text-dark-800` : `text-white`}`
          }
          `}> {defaultValue || "50.00"}</span>
          {/* <InfoText {...infoText} variant="medium"  titleclassname ="" /> */}
          <>
        
          <div className={`flex infotext medium dark gap-[5px]`}>
          <span className={` text-[#606166] fw-500 fs-12 lh-14`}>{infoText.text}</span>
            <>
              <img
                src={`${currentIcon === infoText.text ? "/images/info-green-icon.svg" : "/images/Maskd (2).svg"}`}
                alt=""
                width={10}
                height={10}
                id={infoText.text}          
        ...
        <div>
          <div className={`px-[16px] py-[5px] w-[80px] h-[30px] flex justify-center items-center ${theme === 'dark' ? "bg-white text-dark-800" : "text-white bg-[#3F4048]"}`}>
